...
const settingsNames = {
  ...
  ROOM_CONFERENCE_AUDIO: "room.conferenceAudio",
  ROOM_CONFERENCE_VIDEO: "room.conferenceVideo",
  ROOM_CURRENT_V_PDF: "room.currentVPDF",
  ROOM_LARGE_PLAYER_USER_ID: "room.largePlayerUserId",
};
...
export default function() {
  ...
  const roomSettings = computed(() => ({
    ...
    enabledConferenceAudio: room.settings.conferenceAudio,
    enabledConferenceVideo: room.settings.conferenceVideo,
    enabledVPDF: Boolean(room.settings.currentVPDF),
    enablePlayerUserId: room.settings.largePlayerUserId,
  }));
  ...
  return {
    ...
    roomEnableSoundMessageChatPublic: computed({
      ...
      set: (vl) =>
        changeSetting(settingsNames.ROOM_SOUND_PUBLIC_CHAT_MESSAGE, vl),
    }),
    roomEnableSoundMessageChatPrivate: computed({
      get: () => roomSettings.value.enabledSoundMessageChatPrivate,
      set: (vl) =>
        changeSetting(settingsNames.ROOM_SOUND_PRIVATE_CHAT_MESSAGE, vl),
    }),
    ...
    roomEnableConferenceAudio: computed({
      get: () => roomSettings.value.enabledConferenceAudio,
      set: (vl) => {
        changeSetting(settingsNames.ROOM_CONFERENCE_AUDIO, vl);
        if (!vl && !roomSettings.value.enabledConferenceVideo) {
          changeSetting(settingsNames.ROOM_CONFERENCE_VIDEO, true);
        }
      },
    }),
    roomEnableConferenceVideo: computed({
      get: () => roomSettings.value.enabledConferenceVideo,
      set: (vl) => {
        changeSetting(settingsNames.ROOM_CONFERENCE_VIDEO, vl);
        if (!vl && !roomSettings.value.enabledConferenceAudio) {
          changeSetting(settingsNames.ROOM_CONFERENCE_AUDIO, true);
        }
      },
    }),
    ...
    roomLargePlayerUserId: computed({
      get: () => roomSettings.value.enablePlayerUserId,
      set: (vl) => changeSetting(settingsNames.ROOM_LARGE_PLAYER_USER_ID, vl),
    }),
  };
}
