...
export default function usePresentationPaging() {
    ...
    const {isHost, isParticipant, participantIsNotCalled, participantIsCalled} = useCurrentUser()

    const selectPresentationSlide = index => hasPage(index)
        ? slideCurrentPresentation(index, isParticipant.value && participantIsNotCalled.value) : undefined
    ...
    const showPagination = computed(() => totalPages.value > 1 && (isHost.value || participantIsCalled.value || roomEnableSwitchSlide.value))
