...
export default function usePlayback() {
    ...
    const time = computed(() => ({
        current: int2time(timeline.value / Math.pow(10, 6)),
        duration: int2time(duration.value / Math.pow(10, 6))
    }))

    const play = () => {
        // if (!waiting.value) {
        clearInterval()
        setInterval(() => timeline.value += Math.pow(10, 5), 100, false)
        // }
    }
    ...
    return {
        ...
        progress: computed(() => {
            return Math.round(timeline.value / duration.value * 100)
        })
    }
}