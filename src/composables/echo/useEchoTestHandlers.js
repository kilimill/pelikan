...
export default function useEchoTestHandlers() {
  ...
  const channelPrivate = store.getters["application/channelPrivate"]
  ...
  watch(mediaStreamEcho, newVal => {
    if (typeof newVal === 'object') {
      janusService.createEchoTestOffer(newVal, currentBitrate.value)
    }
  })
  ...
  const checkMessages = () => {
    let messages = new Promise((resolve) => {
      messageService.subscribe(channelPrivate, subscription => {
        subscription.on('connect', () => {
          roomApi.sendEchoTestMessages(channelPrivate, 2);
        });
  ...
  const publishMedia = async () => {
    ...
    getUserMedia(constr).then((stream) => {
      ...
      janusService.clearEchoPlayer();
      handleGUMErrors(error);
    });
  }

  const echoTestPluginLoadCallback = async () => {
    await publishMedia()
  }

  const sourceChangeEvent = async () => {
    if (store.getters['echoTest/finished']) {
      store.commit('echoTest/setAlertShow', false);
      store.commit('echoTest/setFailed', false);
