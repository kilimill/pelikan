...
export default function useJanusHandlers(){
  ...
  const {confActive, connectedUsers} = useConference()
  const {getPlayer} = useConferencePlayers()
  ...
  const initJanus = () => {
    ...
    janusService.initJanus(() => {
      if (currentRoom.value.isActive) {
        ...
        initJanusOutPrimary();
        if (confActive.value && connectedUsers.value.length > 0) {
          connectedUsers.value.forEach((uid) => {
            let mp = mountpointUser(uid).value
            let player = getPlayer(uid)
            initJanusOutCalled(uid, mp, player);
          })
        }
      }
  ...
  const initJanusOutCalled = (userId, mpId, player) => {
    janusService.initJanusOutCalled(userId, mpId, player, janusOutCalledReconnectCallback).then(janusOutReadyCallback).catch((err) => {
      console.error('janus out init error', err);
    });
  }
