...
export default function useBrushTool() {
    ...
    const calcSize = (canvasWidth) => {
        return Math.floor(unref(tool.size) * (canvasWidth / 1000))
    }
    ...
    return {
        ...
        pushState: ({
            x,
            y
        }) => privateDrawingContext.path.push({
            x,
            y
        }),
        fillState: ({
            path
        }) => privateDrawingContext.path = Array.isArray(path) ? [...path.map(i => ({
        ...
        draw() {
            ...
            canvasContext.lineWidth = calcSize(rect.width)
