...
const settingsNames = mapValues({
    ...
    USER_BLOCK_AUDIO: 'user.blockAudio',
    USER_BLOCK_VIDEO: 'user.blockVideo',
}, value => value.replace(/^[^.]+\./, ''))
...
export default function (userId) {
    ...
    const userSettings = computed(() => ({
        enabledAudio: Boolean(currentUser.settings.audioOn),
        enabledVideo: Boolean(currentUser.settings.videoOn),
        ...
        allowedAudio: !user?.settings[settingsNames.USER_BLOCK_AUDIO],
        allowedVideo: !user?.settings[settingsNames.USER_BLOCK_VIDEO],
    }))
    ...
    return {
        ...
        userAllowAudio: computed({
            get: () => userSettings.value.allowedAudio,
            set: vl => changeSetting(settingsNames.USER_BLOCK_AUDIO, !vl)
        }),
        userAllowVideo: computed({
            get: () => userSettings.value.allowedVideo,
            set: vl => changeSetting(settingsNames.USER_BLOCK_VIDEO, !vl)
        }),
    }
}