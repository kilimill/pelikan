      ...
      <section class="miro-boards--list">
        <div class="list-item" v-for="board of miroBoards" :key="board.id">
            <div class="item-actions">
              ...
              <button  v-if="actionsEnabled" name="delete" type="button" class="btn action-button" @click="deleteItem = board">
                <span class="material-icons material-icons--delete-item"></span>
              </button>
              <input type="hidden" value="1qwe2qaz" id="boardURL" />
              <button
                  v-if="actionsEnabled || board.enabled"
                  name="download"
                  type="button"
                  class="btn action-button btn-download"
                  title="Скопировать URL доски в буфер обмена"
                  @click="copyText(board)"
              >
                <span class="material-icons">download</span>
              </button>
            </div>
            <span v-if="actionsEnabled || board.enabled" class="item-name">{{ board.name }}</span>
        </div>
      </section>
    </template>
    ...
    <modal-dialog ref="miroModal" centered>
      ...
      <template v-slot:modal-body>
        <form name="miro-form" @submit.prevent>
          <div class="mb-3">
            <label for="miro-name" class="form-label" :class="{'text-alert': hasNameErrors}">Название</label>
          ...
          <div class="mb-3">
            <label for="miro-link" class="form-label" :class="{'text-alert': hasLinkErrors}">Ссылка</label>
