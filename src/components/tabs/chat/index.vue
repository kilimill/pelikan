<template>
  <div class="tab-content" ref="content" v-if="chatAccess">
    <header class="chat-header chat-header--public" v-show="isPublicChat">
      <div class="btn-group btn-group_tabs" role="group">
        ...
        <template v-if="currentUser.isHost && !isPlayback">
    ...
    <header class="chat-header chat-header--private" v-show="isPrivateChat">
      ...
      <section class="chat-header-title" >
        <span>Личный чат: </span><p class='chat-header-name' :data-bs-title="dialogName" v-text="dialogName"></p>
      </section>
    </header>
...
export default {
  ...
  setup(props) {
    ...
    const {currentUser, isHost, isParticipant} = useCurrentUser()
    ...
    return {
      ...
      playbackSettings: reactive({
        permissionChatPublic: playbackPermissionChatPublic,
      }),
    }
  },
  data: () => ({
  ...
  computed: {
    ...
    chatAccess(){
      return !this.isPlayback ||
          this.currentUser.isHost || this.playbackSettings.permissionChatPublic
    },
    participantUserId() {
  ...
  methods: {
    ...
    onScroll({scrollPercent}) {
      this.autoScroll = Math.round(scrollPercent) === 100
    },
  },
  watch: {
    messages: {
