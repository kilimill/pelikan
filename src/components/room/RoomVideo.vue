    ...
    <div class="camera-window">
      <div class="camera-video" :class="{ play: showPlayer }">
        <span class="camera-video-text" v-show="currentRoom.isPending">
          {{
            currentUser.isHost
              ? "Для начала трансляции мероприятия нажмите кнопку Старт ►"
              : "Мероприятие скоро начнётся"
          }}
        </span>
        <span class="camera-video-text" v-show="currentRoom.isPaused"
          >Мероприятие приостановлено</span
        >
        <span class="camera-video-text" v-show="currentRoom.isFinished"
          >Мероприятие завершено</span
        >
        ...
        <div v-show="showPlayer" id="video-wrapper-block">
          ...
          <div ref="playerCanvasWrapper" id="main-player-canvas-wrapper">
            <div ref="playerCanvasAreaWrapper" id="main-player-canvas-area-wrapper">
              <canvas ref="playerCanvas" id="main-player-canvas"> </canvas>
            </div>
          </div>
        </div>
      </div>
      <div class="camera-video-full camera-video-btn-panel">
        <drop-down-button
          ...
          :options="videoSourcesList"
          v-show="currentRoom.isIpCamRoom"
        />
        <!-- <button
        ...
        >
          <span class="material-icons">{{
            roomSettings.enableSwitchVideo ? "code" : "code_off"
          }}</span>
        </button> -->

        <!-- <button
          v-show="showMediaToggleIcons"
          class="btn mic-off"
          :data-bs-title="messages.audioOn"
        ...
        <button
          v-show="showMediaToggleIcons"
          class="btn camera-off"
          :data-bs-title="messages.videoOn"
...
export default {
  ...
  props: { placement: String, efwweef: String },
  data() {
    return {
  ...
  setup(props) {
    ...
    const { isLargePlace, isSmallPlace, switchPlaces } = usePlacesInjection(
  ...
  computed: {
    ...
    ...mapGetters("room/streams", ["activeStreams", "allStreams"]),
    ...mapGetters("media", [
    ...
    videoSourcesList() {
      ...
      if (this.currentRoom.isWebCamRoom && this.hostVideoOn) {
        arr.push({ value: this.mountPoints.web?.host.id, name: "Ведущий" });
      }
      ...
      if (
        ...
        arr.push({
          value: this.mountPoints.web.called.id,
          name: this.calledUser.alias,
        });
    ...
    userAffectedByCallControl() {
      return this.currentUser.isParticipant && this.currentUser.isNotCalled;
    },
    affectedByCallControl() {
      return (
        this.roomSettings.enabledUserCallControl &&
    ...
    selectedSource(newVal, oldVal) {
      if (oldVal !== undefined && newVal !== undefined) {
        if (this.currentUser.isHost) {
          ...
          this.store.commit("room/settings/changeSetting", {
  ...
  methods: {
    ...mapMutations("room/streams", ["toggleStream"]),
    // audioButtonHandler() {
    //   if (this.currentRoom.isIpCamRoom && this.currentUser.isHost) {
