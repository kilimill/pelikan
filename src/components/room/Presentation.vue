<template>
  ...
  <div class="content-controls presentation-controls" ref="presentationControls">
    <div
      id="presentation-paginator-block"
      v-if="mode !== 'mode.playback'"
      v-show="showPagination && isLargePlace"
      class="btn-group"
      role="group"
    >
      <button
    ...
    <div id="presentation-actions-block" class="btn-group" role="group">
      <div class="btn-group" v-show="isLargePlace" v-if="mode !== 'mode.playback'">
        <div class="btn-group" v-if="currentUser.isHost">
          <!-- <input
            type="checkbox"
            class="btn-check"
            id="presentation-strict-mode"
            autocomplete="off"
            v-model="strictMode"
          />
          <label
            class="btn btn-outline-transparent"
            for="presentation-strict-mode"
...
export default {
  ...
  setup() {
    ...
    let mode = store.state.application.mode;
    const { presentationImage } = usePresentationImage();
    const { switchPlaces, isLargePlace, isSmallPlace } = usePlacesInjection();
    const { roomEnableDrawModeUserCall } = useRoomSettings();

    const currentUser = computed(() => store.getters["user/currentUser"]);
    const userCanDraw = computed(
      () =>
        currentUser.value.isHost ||
        (currentUser.value.isCalled && roomEnableDrawModeUserCall.value)
    );
