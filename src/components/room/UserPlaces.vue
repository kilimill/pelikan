<template>
  <teleport
    v-for="pair of placesPairs"
    :key="pair['name']"
    :to="`#room-place-${pair['place']}`"
  >
    <component
      :is="pair['name']"
      :data-placement="pair['place']"
      :placement="pair['place']"
    />
  </teleport>
</template>
...
export default {
  ...
  setup() {
    ...
    const switchPlaces = () =>
      currentUser.value.isHost ? hardSwitchPlaces() : softSwitchPlaces();

    provide("switchPlaces", switchPlaces);

    watch(
      () => roomSettings.value.enabledVPDF,
      (value) => (placesSwitch.value = Number(value)),
      { immediate: true }
    );
