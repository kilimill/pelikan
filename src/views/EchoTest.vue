<template>
  <div id="echo-test-page">
    <div id="echo-test-block">
      ...
      <p v-if="mustPassEchoTest" class="top-text">
        Включите в браузере доступ к вашему оборудованию
      </p>
      <p class="device-support">
        <span :class="{ 'no-device': !hasWebRTC }">
          <span class="material-icons">{{
            hasWebRTC ? "check_circle_outline" : "highlight_off"
          }}</span>
          Поддержка видео и аудио
        </span>
        <span :class="{ 'no-device': !hasMessages }">
          <span class="material-icons">{{
            hasMessages ? "check_circle_outline" : "highlight_off"
          }}</span>
          Поддержка сообщений
        </span>
      </p>
      <div>
        ...
        <div class="connect-buttons">
          <button @click="goToRoom()" :disabled="isDisableConnectButton">
            Подключиться
          </button>
  ...
  <modal-window ref="modalEchoTestError" id="modalEchoTestError" type="alert">
...
export default {
  ...
  setup() {
    ...
    const { messageSub } = useMessageSubscription(
  ...
  computed: {
    ...mapGetters("media", ["listAudio", "listVideo"]),
  },
  watch: {
    modalShow(newVal) {
      if (newVal) {
