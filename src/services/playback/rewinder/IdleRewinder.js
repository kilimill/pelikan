...
export default class IdleRewinder extends Rewinder {
    ...
    process(receiver, resolve, reject) {
        ...
        request.addEventListener('success', () => {
            ...
            if (value) {
                ...
                this.#processEvent(value, firstMessage, isLast, idle)
                receiver(value)
     ...
     */
    #processEvent(event, first, isLast, idleTime) {
        if (event === first) {
            this.manager.idleTime.timeStart = event.time_created
        }
        if (isLast) {
            let t = this.isStartEvent(event) ? first.time_created : idleTime
            this.manager.idleTime.idleTime = t - event.time_created
        }
    }
