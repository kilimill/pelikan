...
export default class PlaybackService extends EventEmitter {
    ...
    addEventSource(eventSource) {
        ...
        eventSource.addEventListener(EVENT_SEEKING, event => {
            this.#rewinder.idleTime.rewindForward = true
            if (times.value > event.seek) {
                this.#rewinder.idleTime.saveFirst()
                this.#rewinder.idleTime.rewindForward = false
            }
            this.#rewinder.rewind(event)
            this.dispatchEvent(EVENT_SEEKING, event)
            
        })
        eventSource.addEventListener(EVENT_TIMEUPDATE, ({currentTime}) => {
            times.value = currentTime
            this.#rewinder.rewind({time: currentTime})
        })  
    }
    ...
    #dispatch() {
        ...
        this.#middleware.addEventListener(EVENT_RETURN, event => {
            this.#rewinder.rewind(event)
    
            this.#rewinder.idleTime.rewindForward = true
            this.#rewinder.rewind({seek: times.value})
        })
        ...
        this.#middleware.addEventListener(EVENT_FINISH, () => {
            ...
            this.#rewinder.releaseProcessing()
    ...
    #dispatchMessage(messageEvent) {
        ...
        this.middleware.dispatchMessage(messageEvent)
            .then(message => {if(message) {this.dispatchEvent('message', message.data)}
        }
                // if (message) {
