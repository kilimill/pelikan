...
export default {
    ...
    getters: {
        currentPresentation: state => state.list.find(i => i.id === state.current),
    ...
    actions: {
        ...
        selectPresentation: (context, {presentationId, slideIndex}) => {
            if(context.rootGetters["room/currentRoom"].isPlayback){
                return true;
            }
    ...
    mutations: {
        ...
        presentationSelect(state, {presentationId, slideIndex}) {
            if (DEFAULT_PRESENTATION_ALIAS !== (state.current = presentationId)) {
                let pres = state.list.find(i => i.id === presentationId);
                if(pres){
                    pres.slideIndex = Number(slideIndex)
                }
            }
        },
